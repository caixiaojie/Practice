...
package com.yanyu.practice.materialdialog
...
class MaterialDialogMainActivity : AppCompatActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    setContentView(R.layout.activity_main_material_dialog)

    R.id.basic.onClickDebounced {
      MaterialDialog(this).show {
        message(R.string.shareLocationPrompt)
        debugMode(debugMode)
        lifecycleOwner(this@MaterialDialogMainActivity)
      }
    }
  ...
  private fun showFileChooser() = runWithPermissions(READ_EXTERNAL_STORAGE) {
    MaterialDialog(this).show {
      fileChooser(this@MaterialDialogMainActivity) { _, file ->
  ...
  private fun showFileChooserButtons() = runWithPermissions(WRITE_EXTERNAL_STORAGE) {
    MaterialDialog(this).show {
      fileChooser(context = this@MaterialDialogMainActivity, allowFolderCreation = true) { _, file ->
        toast("Selected file: ${file.absolutePath}")
      }
      negativeButton(android.R.string.cancel)
      positiveButton(R.string.select)
  ...
  private fun showFileChooserFilter() = runWithPermissions(READ_EXTERNAL_STORAGE) {
    MaterialDialog(this).show {
      fileChooser(context = this@MaterialDialogMainActivity, filter = { it.extension == "txt" }) { _, file ->
        toast("Selected file: ${file.absolutePath}")
      }
      debugMode(debugMode)
      lifecycleOwner(this@MaterialDialogMainActivity)
  ...
  private fun showFolderChooserButtons() = runWithPermissions(WRITE_EXTERNAL_STORAGE) {
    MaterialDialog(this).show {
      folderChooser(context = this@MaterialDialogMainActivity, allowFolderCreation = true) { _, folder ->
  ...
  private fun showFolderChooserFilter() = runWithPermissions(READ_EXTERNAL_STORAGE) {
    MaterialDialog(this).show {
      folderChooser(
          context = this@MaterialDialogMainActivity, filter = { it.name.startsWith("a", true) }) { _, folder ->
        toast("Selected folder: ${folder.absolutePath}")
      }
      debugMode(debugMode)
      lifecycleOwner(this@MaterialDialogMainActivity)
